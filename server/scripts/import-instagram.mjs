/**
 * Instagram pipeline: InstaCrawler CSV → clean → download images → platforms/ig.json → merged-data.json
 */
import fs from 'fs'
import path from 'path'
import { fileURLToPath } from 'url'
import { parse } from 'csv-parse/sync'
import { mergePosts, cleanPost, normalizeRow } from './lib/ig-pipeline.mjs'
...
const PLATFORM = 'ig'
const instaCrawlerDir = path.resolve(__dirname, '../../../InstaCrawler')
const platformsDir = path.resolve(__dirname, '../data/platforms')
const platformOutPath = path.join(platformsDir, 'ig.json')
const mergedDataPath = path.resolve(__dirname, '../data/merged-data.json')
const statsPath = path.resolve(__dirname, '../data/import-stats-ig.json')

const DEFAULT_BATCHES = [
  { file: path.join(instaCrawlerDir, 'penang_ig.csv'), label: 'penang' },
  { file: path.join(instaCrawlerDir, 'penang_ig2.csv'), label: 'penang' },
]
...
function inferBatchLabel(filePath) {
  ...
  if (base.includes('kuala') || /(?:^|[^a-z])kl(?:[^a-z]|$)/.test(base)) return 'kl'
  return null
}
...
async function main() {
  ...
  const batches =
    cliFiles.length > 0
      ? cliFiles.map((file, i) => ({ file, label: inferBatchLabel(file) || `batch-${i + 1}` }))
      : DEFAULT_BATCHES
  ...
  if (!skipImages) {
    console.log('\nDownloading post cover images (Instagram CDN)…')
  ...
  console.log(`\nInstagram (${PLATFORM}): ${allKept.length} → ${platformMerged.length} unique`)
