/**
 * Batch enrich + Google covers for Melaka places needing work.
 */
import fs from 'fs'
...
const todo = places
  .filter((p) => p.state === 'Melaka' && !skipIds.has(p._id))
  .sort((a, b) => (b.totalLikes || 0) - (a.totalLikes || 0))

console.log(`Melaka skip (done): ${skipIds.size}`)
console.log(`Melaka todo: ${todo.length}`)
