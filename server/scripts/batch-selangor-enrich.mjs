/**
 * Batch enrich + Google covers for Selangor places needing work.
 */
import fs from 'fs'
...
const todo = places
  .filter((p) => p.state === 'Selangor' && !skipIds.has(p._id))
  .sort((a, b) => (b.totalLikes || 0) - (a.totalLikes || 0))

console.log(`Selangor skip (done): ${skipIds.size}`)
console.log(`Selangor todo: ${todo.length}`)
