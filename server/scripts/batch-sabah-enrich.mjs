/**
 * Batch enrich + Google covers for Sabah places needing work.
 */
import fs from 'fs'
...
const todo = places
  .filter((p) => p.state === 'Sabah' && !skipIds.has(p._id))
  .sort((a, b) => (b.totalLikes || 0) - (a.totalLikes || 0))

console.log(`Sabah skip (done): ${skipIds.size}`)
console.log(`Sabah todo: ${todo.length}`)
