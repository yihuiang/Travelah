/**
 * Batch enrich + Google covers for Pahang places needing work.
 */
import fs from 'fs'
...
const todo = places
  .filter((p) => p.state === 'Pahang' && !skipIds.has(p._id))
  .sort((a, b) => (b.totalLikes || 0) - (a.totalLikes || 0))

console.log(`Pahang skip (done): ${skipIds.size}`)
console.log(`Pahang todo: ${todo.length}`)
