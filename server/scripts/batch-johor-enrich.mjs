/**
 * Batch enrich + Google covers for Johor places needing work.
 */
import fs from 'fs'
...
const todo = places
  .filter((p) => p.state === 'Johor' && !skipIds.has(p._id))
  .sort((a, b) => (b.totalLikes || 0) - (a.totalLikes || 0))

console.log(`Johor skip (done): ${skipIds.size}`)
console.log(`Johor todo: ${todo.length}`)
