/**
 * Batch enrich + Google covers for Kuala Lumpur places needing work.
 */
import fs from 'fs'
...
const todo = places
  .filter((p) => p.state === 'Kuala Lumpur' && !skipIds.has(p._id))
  .sort((a, b) => (b.totalLikes || 0) - (a.totalLikes || 0))

console.log(`KL skip (done): ${skipIds.size}`)
console.log(`KL todo: ${todo.length}`)
