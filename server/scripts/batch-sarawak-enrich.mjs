/**
 * Batch enrich + Google covers for Sarawak places needing work.
 */
import fs from 'fs'
...
const todo = places
  .filter((p) => p.state === 'Sarawak' && !skipIds.has(p._id))
  .sort((a, b) => (b.totalLikes || 0) - (a.totalLikes || 0))

console.log(`Sarawak skip (done): ${skipIds.size}`)
console.log(`Sarawak todo: ${todo.length}`)
