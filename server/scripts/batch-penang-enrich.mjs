/**
 * Batch enrich + Google covers for all Penang places needing work.
...
const todo = places
  .filter((p) => p.state === 'Penang' && !skipIds.has(p._id))
  .sort((a, b) => (b.totalLikes || 0) - (a.totalLikes || 0))

console.log(`Penang skip (done): ${skipIds.size}`)
console.log(`Penang todo: ${todo.length}`)
