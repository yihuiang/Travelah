/**
 * Batch enrich + Google covers for Perak places needing work.
 * Skips places already enriched with local covers.
 */
import fs from 'fs'
...
const skip = places.filter(
  (p) =>
    ...
    !/douyinpic|rednotecdn|xhscdn|tiktok|fbcdn/i.test(p.coverImage || '') &&
    (p.googleRating != null || p.openingHours?.length || p.googleOpeningHours?.length || p.openingHoursSource),
)

const skipIds = new Set(skip.map((p) => p._id))
const todo = places
  .filter((p) => p.state === 'Perak' && !skipIds.has(p._id))
  .sort((a, b) => (b.totalLikes || 0) - (a.totalLikes || 0))

console.log(`Perak skip (done): ${skipIds.size}`)
console.log(`Perak todo: ${todo.length}`)
