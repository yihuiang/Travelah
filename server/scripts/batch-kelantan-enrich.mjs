/**
 * Batch enrich + Google covers for Kelantan places needing work.
 */
import fs from 'fs'
...
const todo = places
  .filter((p) => p.state === 'Kelantan' && !skipIds.has(p._id))
  .sort((a, b) => (b.totalLikes || 0) - (a.totalLikes || 0))

console.log(`Kelantan skip (done): ${skipIds.size}`)
console.log(`Kelantan todo: ${todo.length}`)
