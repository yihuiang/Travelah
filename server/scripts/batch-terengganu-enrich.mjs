/**
 * Batch enrich + Google covers for Terengganu places needing work.
 */
import fs from 'fs'
...
const todo = places
  .filter((p) => p.state === 'Terengganu' && !skipIds.has(p._id))
  .sort((a, b) => (b.totalLikes || 0) - (a.totalLikes || 0))

console.log(`Terengganu skip (done): ${skipIds.size}`)
console.log(`Terengganu todo: ${todo.length}`)
